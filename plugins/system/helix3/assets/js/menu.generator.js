...
jQuery(function ($) {
	...
	$.fn.columnSort = function () {
		$(this)
			.sortable({
				...
				start: function (event, ui) {
					var plus;
					if (ui.item.hasClass("sp-col-sm-1")) plus = "sp-col-sm-1";
					else if (ui.item.hasClass("sp-col-sm-2")) plus = "sp-col-sm-2";
					else if (ui.item.hasClass("sp-col-sm-3")) plus = "sp-col-sm-3";
					else if (ui.item.hasClass("sp-col-sm-4")) plus = "sp-col-sm-4";
					else if (ui.item.hasClass("sp-col-sm-5")) plus = "sp-col-sm-5";
					else if (ui.item.hasClass("sp-col-sm-7")) plus = "sp-col-sm-7";
					else if (ui.item.hasClass("sp-col-sm-8")) plus = "sp-col-sm-8";
					else if (ui.item.hasClass("sp-col-sm-9")) plus = "sp-col-sm-9";
					else if (ui.item.hasClass("sp-col-sm-10")) plus = "sp-col-sm-10";
					else if (ui.item.hasClass("sp-col-sm-11")) plus = "sp-col-sm-11";
					else if (ui.item.hasClass("sp-col-sm-12")) plus = "sp-col-sm-12";
					else plus = "sp-col-sm-6";
	...
	$("#menuWidth").change(function () {
		...
		if (width >= 200) {
			$("#megamenulayout").css("width", width).data("width", width);
		} else {
	...
	$(document).on("click", "#layout-modal a", function (event) {
		...
		if (!String.prototype.format) {
			String.prototype.format = function () {
				...
				return this.replace(/{(\d+)}/g, function (match, number) {
					return typeof args[number] != "undefined" ? args[number] : '<div class="modules-container"></div>';
				});
			};
		}
		...
		if (newLayout.length === 1) {
			...
			var newLayoutHtml = designString.format(
				colHtml[0],
				colHtml[1],
				colHtml[2],
				colHtml[3],
