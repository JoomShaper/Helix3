...
class Helix3Menu
{
    ...
    public function initMenu()
    {
        ...
        foreach ($items as &$item) {
            ...
            $parent                           = isset($this->children[$item->parent_id]) ? $this->children[$item->parent_id] : [];
            $parent[]                         = $item;
        ...
        foreach ($items as &$item) {
            ...
            $item->megamenu = ($item->getParams()->get('megamenu')) ? $item->getParams()->get('megamenu') : 0;
    ...
    private function getItem($item)
    {
        ...
        $this->menu .= $this->start_el(['item' => $item]);
    ...
    {
        $items     = isset($this->children[$item->id]) ? $this->children[$item->id] : [];
        ...
        $mega_json = $item->getParams()->get('menulayout');
        if ($mega_json) {
            ...
            $layout  = $layout[0];
            $col     = $layout->attr[0];
            $mod_ids = ($col->moduleId) ? explode(',', $col->moduleId) : [];
    ...
    private function mega($item)
    {
        ...
        foreach ($layout as $row) {
            ...
            foreach ($row->attr as $col) {
                ...
                if (count($items)) {
                    $item_ids = ($col->menuParentId) ? explode(',', $col->menuParentId) : [];
                    ...
                    if (count($item_ids)) {
                        ...
                        foreach ($item_ids as $item_id) {
                            if (! empty($this->_items[$item_id])) {
                                ...
                                $items = isset($this->children[$item_id]) ? $this->children[$item_id] : [];
    ...
    private function start_el($args = [])
    {
        $item  = $args['item'];
        $class = 'sp-menu-item';
        ...
        if ($custom_class = $item->getParams()->get('class')) {
            $class .= ' ' . $custom_class;
        }
    ...
    private function item($item, $extra_class = '')
    {
        ...
        //Hide Link Title
        if (! $showmenutitle = $item->getParams()->get('showmenutitle', 1)) {
            $linktitle = '';
        }

        //Add Menu Icon
        if ($icon = $item->getParams()->get('icon')) {
            if ($showmenutitle) {
        ...
        if ($item->getParams()->get('menu_show', 1) != 0) {
            switch ($item->browserNav) {
                default:
                case 0:
                    $link_rel  = ($item->getParams()->get('menu-anchor_rel', '')) ? 'rel="' . $item->getParams()->get('menu-anchor_rel') . '"' : '';
                    $flink     = ($flink) ? $flink : 'javascript:void(0);';
                    $output   .= '<a ' . $class . ' href="' . $flink . '" ' . $link_rel . ' ' . $title . '>' . $linktitle . '</a>';
                    break;
                case 1:
                    $link_rel  = ($item->getParams()->get('menu-anchor_rel', '') == 'nofollow') ? 'noopener noreferrer nofollow' : 'noopener noreferrer';
