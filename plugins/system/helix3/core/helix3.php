...
class Helix3
{
    ...
    {
        //Remove Classes name
        $class_remove = ['layout-column', 'column-active', 'col-sm-'];
     ...
     * @return self
     */
    public static function addCSS($sources, $attribs = [])
    {
    ...
    {
        self::getInstance()->less()->setVariables([$name => $value]);
     ...
     * @return self
     */
    public static function addLess($less, $css, $attribs = [])
    {
        $template  = Factory::getApplication()->getTemplate();
        $themepath = JPATH_THEMES . '/' . $template;
            ...
            if (File::exists($js_file)) {
                if (! self::excludeJS($key, $excludes)) {
                    ...
                    if (self::isMinifiedJS($js_file)) {
                        ...
                        $compressed = \JShrink\Minifier::minify(file_get_contents($js_file), ['flaggedComments' => false]);
                    }
